...
public class GameManager : MonoBehaviour
{
    ...
    public List<NotebookBehavior> evidenceDeck = new List<NotebookBehavior>();
   ...
   //This will contain the cards in our hand
    ...
    public void DrawCard()
    {
        if(deck.Count >= 1)
        {
            ...
            NotebookBehavior randEvidence = evidenceDeck[Random.Range(0, deck.Count)]; 
            ...
            for(int i = 0; i < availableCardsSlots.Length; i++)
            {
                if (availableCardsSlots[i] == true)
                {
                    ...
                    randEvidence.enabled = true; 
                    Debug.Log("We gained a piece of evidence"); 
    ...
    private void Update()
    {
        ...
        if(deck.Count == 0 && discardPile.Count == 7)
        {
            EnableVote(); 
        }
